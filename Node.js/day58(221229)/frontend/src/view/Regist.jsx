...
export default function Regist(props) {
    ...
    const onClickRegist = () => {
        console.log(userid, email, password, passwordCheck, year, month, day, gender);

        if(!userid) return alert("아이디를 반드시 입력해야 합니다.");
        if(!email) return alert("이메일은 반드시 입력해야 합니다.");
        if(!password) return alert("비밀번호를 반드시 입력해야 합니다.");
        if(!passwordCheck) return alert("비밀번호 확인은 필수 입력사항 입니다.");
        if(!year) return alert("연도를 반드시 선택해야 합니다.");
        if(!month) return alert("월을 반드시 선택해야 합니다.");
        if(!day) return alert("일을 반드시 선택해야 합니다.");
        if(!gender) return alert("성별을 반드시 선택해야 합니다.");

        if(password !== passwordCheck) {
            return alert("비밀번호롸 비밀번호 확인의 값이 동일해야 합니다.")
        }
        ...
        if (!email.match(regExp)) {
            return alert("이메일 형식과 맞지 않습니다. 이메일 형을 확인하세요")
        }
    ...
    return (
        <div className="regist-layer">
            ...
            <div className="card">
                ...
                <div className="body">
                    {/* 휴대폰 및 비밀번호 폼 */}
                    <Input type="text" placeholder="사용자 아이디" onChange={onChangeUserid}/>
                    <Input type="text" placeholder="사용자 이메일" onChange={onChangeEmail}/>
